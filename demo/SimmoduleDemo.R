...
simulateRun <- function(nRun, simenv) {
	...
	simulate_qualification <- function(){
		...
		#generate propensities for scenario testing
		numPeople <- length(simframe[[1]])
		qualificationmodels <- propensityModels[["qualification"]]
		qualificationPropensities <- predictOrdinal(qualificationmodels, numPeople, envir=simframe, stochastic=TRUE)
		qualification <<- adjustVar(qualification, "qualification", propens=qualificationPropensities[,-ncol(qualificationPropensities)])
	...
	for (iteration in 1:NUM_ITERATIONS) {
		#iteration = 17
		sfCat("Run", simenv$num_runs_simulated+1, "year", iteration, "\n")
...
map_outcomes_to_run_results <- function(run, simframe, outcomes, cat.adjustments) {
	#have added cat.adjustments and run here 		
	
	dict_demo <- as.list(dict_demo)
	
	outcomes <- outcomes[[run]]

	#to get to work with newer simario version of map_outcomes_to_run_results
	catvars <- getOutcomeVars(simframe, "categorical", "demo")	
	#yet to make these parameters do anything in this function
	...
	#browser()
	#user specified subgroup variable
	#prepend "outcomes$", "simframe$", or "env.base$...outcomes$" to variables so theyprepend.paths
	#can be found
	if (!is.null(attr(cat.adjustments[[1]], "logisetexpr"))) {
		subgroup.expr <- attr(cat.adjustments[[1]], "logisetexpr")
		prepended.exprs <- prepend.paths(subgroup.expr) 
	...
	if (!is.null(attr(cat.adjustments[[1]], "logisetexpr"))) {
		run_results$freqs_by_subgroup <- sfLapply(outcomes[catvars], table_mx_cols_MELC_list, grpby=sg.var, grpby.tag=sg.expr, dict=dict_demo)
		
		run_results$freqs_by_subgroup_base_data <- sfLapply(base.outcomes.current.run[catvars], table_mx_cols_MELC_list, grpby=sg.var.base, grpby.tag=sg.expr.base, dict=dict_demo)
		
		run_results$means_by_subgroup <- sfLapply(outcomes[convars], quantile_mx_cols_BCASO_list, grpby=sg.var, grpby.tag=sg.expr, dict=dict_demo)
		
		run_results$means_by_subgroup_base_data <- sfLapply(base.outcomes.current.run[convars], quantile_mx_cols_BCASO_list, grpby=sg.var.base, grpby.tag=sg.expr.base, dict=dict_demo)
		
		run_results$freqs_continuousGrouped_by_subgroup <- sfLapply(binned.list, table_mx_cols_MELC_list, grpby=sg.var, grpby.tag=sg.expr, dict=dict_demo)
		
		run_results$freqs_continuousGrouped_by_subgroup_base_data <- sfLapply(binned.list.base, table_mx_cols_MELC_list, grpby=sg.var.base, grpby.tag=sg.expr.base, dict=dict_demo)
	}
	

	run_results$freqs <- sfLapply(outcomes[catvars], table_mx_cols_MELC_list, dict=dict_demo)	
	
	run_results$freqs_continuousGrouped <- sfLapply(binned.list, table_mx_cols_MELC_list, dict=dict_demo)
	
	run_results$means <- sfLapply(outcomes[convars], quantile_mx_cols_BCASO_list, dict=dict_demo)
	
	run_results$summaries <- sfLapply(outcomes[convars], summary_mx_cols)
	
	run_results$quantiles <- sfLapply(outcomes[convars], quantile_mx_cols, new.names=c("Min", "20th", "40th", "60th","80th","Max"), probs=seq(0,1,0.2), na.rm = TRUE)
...
#' simframe<-env.base$simframe
#' collate_all_run_results(all_run_results, cat.adjustments, simframe)
collate_all_run_results <- function(all_run_results, cat.adjustments, simframe, outcomes) {
