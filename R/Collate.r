...
label_flattened_mx <- function(mx.flattened, dict, row.dim.label="", col.dim.label="") {
  ...
  colnames(mx.flattened) <- cmatchFlattened(dict, cnames, varname, grpby.tag)
  ...
  if ((num.spaces[1]==0)) {
    ...
    if (!is.null(binbreaks)) {
      if (any(is.na(suppressWarningsas.numeric(sub.col.names)))) {
        ...
        un.ordered.names <- cmatchFlattened(dict, sub.col.names, varname, grpby.tag)
      }
      ...
      colnames(mx.flattened) <- cmatchFlattened(dict, sub.col.names, varname, grpby.tag)
    }
    ...
    if (any(is.na(suppressWarnings(as.numeric(simple.names))))) {
      #names are already the character versions and the match is not needed
      simple.names.words <- simple.names
    } else {
      #convert numeric codes to character names
      simple.names.words <- cmatch(dict, simple.names, varname)
    }
    ...
    if (!is.null(binbreaks)) {
      ...
      if (any(is.na(suppressWarnings(as.numeric(grp.names))))) {
        #names are already the character versions and the match is not needed
        grp.names.words <- grp.names
      } else {
        #no binbreaks
        #convert numeric codes to character names
        grp.names.words <- cmatch(dict, grp.names, grpby.tag)
      }
