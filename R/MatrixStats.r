	...
	if (!is.na(grpby.tag)) {
		...
		for (i in 1:(dim(xa)[3])) {
			...
			if (numgroups<0) {
				...
				if ((numgroups==0)&(dlookup_exists(dict, grpby.tag)==1)) {
...
mean_mx_cols_BCASO <- function (mx, grpby=NULL, grpby.tag=NULL, logiset=NULL, wgts=NULL, dict=NULL) {
	...
	if ( is.null(grpby)) { ##|| sum(is.na(grpby)) == length(grpby) ) {
		...
		if (is.null(dict)) {
			stop("Dict is NULL")
		} else 	if ((!is.null(grpby.tag))&(dlookup_exists(dict, grpby.tag)==1)) {
			colnames(result) <- c("Not in subgroup", "In subgroup")
		}
	}
...
#' wgts<-matrix(c(1,1,1,1,1,2,2,2,2,1), ncol = 2)
#' na.rm = FALSE ; na.rm = TRUE
mean_mx_cols_BCASO_list <- function (mx, grpby=NULL, grpby.tag=NULL, logiset=NULL, wgts=NULL, dict=NULL) {
